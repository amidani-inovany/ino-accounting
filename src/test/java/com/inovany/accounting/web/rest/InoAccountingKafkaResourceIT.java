...
import com.inovany.accounting.config.KafkaProperties;
...
class InoAccountingKafkaResourceIT {
    ...
    void setup() {
        ...
        InoAccountingKafkaResource kafkaResource = new InoAccountingKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/ino-accounting-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ino-accounting-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
